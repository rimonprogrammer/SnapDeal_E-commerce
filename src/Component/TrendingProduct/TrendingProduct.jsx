...
function TrendingProduct({ textColor}) {
    const textLocalColor = JSON.parse(localStorage.getItem('textColor'));

  return (
    <div>
    <div className="container pt-5">
        <h2 style={textLocalColor ? textLocalColor : textColor} className='text-center pt-5' data-aos="fade-up" data-aos-delay='100'>Trending product</h2>
        <p style={textLocalColor ? textLocalColor : textColor} className='text-center' data-aos="fade-up" data-aos-delay='200'>Trending product just for you!</p>
        <div className="all-bast-product">
            {
                Data.filter((element) => element.trending === true)
                .map((product, index) =>{
